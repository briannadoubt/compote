...
struct PushCommand: ParsableCommand {
    ...
    @Argument(help: "Service names to push (all if not specified)")
    var services: [String] = []
    ...
    mutating func run() throws {
        ...
        try runAsyncTask {
            ...
            // Push images
            let servicesToPush = servicesArg.isEmpty ? nil : servicesArg
            try await orchestrator.push(services: servicesToPush)

            logger.info("Image push complete")
        }
    }
}
